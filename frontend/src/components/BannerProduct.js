            ...
            <div className='hidden md:flex h-full w-full overflow-hidden'>
                {
                    desktopImages.map((imageURI, index)=>{
                        return (
                            <div className='w-full h-full min-w-full min-h-full transition-all' key={imageURI} style={{transform : `translateX(-${currentImage * 100}%)`}}>
            ...
            <div className='flex h-full w-full overflow-hidden md:hidden'>
                {
                    mobileImages.map((imageURI, index)=>{
                        return (
                            <div className='w-full h-full min-w-full min-h-full transition-all' key={imageURI} style={{transform : `translateX(-${currentImage * 100}%)`}}>
                                <img src={imageURI} className='w-full h-full object-fill'/>
                            </div>
                        )
                    })
                }
